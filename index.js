...
// delete a saved recipe from a user's account
app.post("/deleteSavedRecipe", async (req, res) => {
  try {
    console.log("req.body:", req.body);
    const Recipe = {
      PersonID: req.session.personID,
    ...
    const deleteRecipeResult = await knex("PersonRecipes").delete(Recipe);
    console.log("deleteRecipeResult", deleteRecipeResult);
    res.redirect("/savedRecipes.ejs");
  } catch (error) {
    console.error("Database Insert Error:", error);
    res.status(500).send("Internal Server Error");
